...
class studentPageRepositoryTest {
    ...
    //test 클래스는 생성자 주입x -> @Autowired 사용
    //보통 final 사용 @requiredargsconstructor
    @Autowired
    ...
    void testBasicPagination() {
        ...
        //페이지 정보 생성
        //페이지 번호가 zero-base -> 0이 1페이지
        Pageable pageable
                = PageRequest.of(pageNo-1, amount,
                //정렬 기준값은 필드명(entity)!! 컬럼명(SQL)!
                //Sort.by("name").descending() //1개 정렬 조건
                Sort.by(
                        //정렬 多조건
                        Sort.Order.desc("name"),//내림차순
                        Sort.Order.asc("city")//오름차순
                )
        );
        ...
        //studentList.forEach(stu -> System.out.println(stu)); // 식 자체 @@ 참조할클래스::메서드이름
        studentList.forEach(System.out::println); //모든 객체를 출력한다
        System.out.println("\n\n\n");
    ...
    void testSearchAndPaging() {
        ...
        System.out.println("next = " + next);
        System.out.println("prev = " + prev);
        students.getContent().forEach(System.out::println); //학생 정보 받고, ,forEach
        System.out.println("\n\n\n");
