...
class DepartmentTest {
    ...
    /*
    * JPA에서 사용하는 entity들을 관리하는 역할을 수행하는 클래스(영속성 컨텍스트를 관리함)
    * 영속성 컨텍스트 : entity를 영구 저장하는 환경 -> Spring Data JPA에서만 사용하지 않음(트랜잭션 마무리하면 사라짐)
    * 영속성 컨텍스느 내의 내용들을 DB에 반영시키거나 비워내거나 수명을 관리 할 수 있는 객체(commit, rollback, flush)
    * */
    @Autowired
    ...
    void testChangeDept() {
        ...
        //entityManager 자주 쓰진 않음 : 실행 시점을 제어할 때
    ...
    @Test
    @DisplayName("N+1 문제 해결 예시") //left JOIN -> JPQL : 상황에 따라 적절하게
